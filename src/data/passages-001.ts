import type { Passage } from "@/types"

// Contextual Short Passages - Batch 1
export const passages: Passage[] = [
  {
    id: 1,
    title: "A Change of Plans",
    text: "Sarah had always dreamed of studying abroad, but she had to abandon her plans when her company offered her a significant promotion. At first, the decision seemed absurd to her friends, who knew how much she wanted to travel. However, Sarah chose to adapt to the new situation. She acknowledged that the opportunity was too good to miss. Her ambition to become a manager had always been strong, and this role would help her attain that goal. She felt genuine excitement as she started her new position, ready to embrace the challenges ahead.",
    wordIds: [1, 3, 9, 7, 16, 33, 186, 145],
    level: "B2",
    topic: "work"
  },
  {
    id: 2,
    title: "The Climate Debate",
    text: "The city council held a debate about carbon emissions and their impact on the environment. One group advocated for stricter rules on factories, while others argued that such measures would be too harsh on local businesses. The discussion was comprehensive, covering every aspect of the problem. Despite the controversy, a consensus was reached: the city would implement new conservation policies over the next five years. Both sides had to compromise, but everyone agreed that the issue was too crucial to ignore.",
    wordIds: [104, 47, 147, 11, 190, 73, 27, 92, 82, 200, 84, 74, 101],
    level: "B2",
    topic: "social"
  },
  {
    id: 3,
    title: "Starting a Business",
    text: "Li Wei had always wanted to be an entrepreneur. After years of working for a large enterprise, he decided to start his own company. His first task was to formulate a clear business plan. He consulted a counsellor who helped him assess the risks. The biggest barrier was funding — his budget deficit was significant. But Li Wei was determined. He managed to forge partnerships with two investors who believed in his vision. Within a year, his small technology company began to flourish, and he knew that his dedication had paid off.",
    wordIds: [156, 154, 178, 87, 98, 29, 37, 111, 177, 174, 110],
    level: "B2",
    topic: "work"
  },
  {
    id: 4,
    title: "A Classroom Discussion",
    text: "Professor Zhang asked her students to contemplate a difficult hypothesis: could artificial intelligence ever develop a conscience? The cognitive complexity of the question made the students pause. One student cited a recent study to support his argument, while another dismissed the idea as an exaggeration. The professor encouraged them to clarify their positions and avoid making arbitrary assumptions. The dialogue lasted the entire class. By the end, the students could not reach a consensus, but the elaborate discussion had enhanced their understanding of the topic considerably.",
    wordIds: [88, 194, 81, 56, 71, 51, 126, 163, 53, 25, 30, 119, 142, 153],
    level: "B2",
    topic: "education"
  },
  {
    id: 5,
    title: "The Heritage Museum",
    text: "The city's heritage museum was struggling with a chronic lack of funding. The council needed to allocate more money to keep it open, but the budget was tight. A local businessman made a generous donation, which helped compensate for the deficit. The museum director was determined to demonstrate the museum's value to the community. She organized an exhibition that illustrated the evolution of the region's ethnic traditions. The response was immense — thousands of visitors came in the first week. The breakthrough convinced the council to commit to long-term support.",
    wordIds: [191, 49, 13, 137, 67, 111, 114, 197, 162, 160, 198, 43, 63],
    level: "B2",
    topic: "daily_life"
  },
  {
    id: 6,
    title: "A Diplomatic Challenge",
    text: "The diplomatic relationship between the two countries had been tense for years. A border dispute was the main source of conflict, and both sides showed aggression in their public statements. A humanitarian crisis in the region made cooperation essential. International organizations tried to facilitate dialogue between the two governments. After months of negotiation, both countries consented to a compromise. They agreed to enforce a ceasefire and confront the underlying issues through peaceful means. The agreement was far from perfect, but it was a crucial first step toward stability.",
    wordIds: [121, 130, 12, 193, 171, 119, 83, 74, 151, 80, 101],
    level: "B2",
    topic: "social"
  },
  {
    id: 7,
    title: "Working from Home",
    text: "When the epidemic forced offices to close, millions of workers had to adapt to working from home. The transition was awkward at first. Many people struggled to maintain discipline and keep a consistent routine. The duration of the situation was uncertain, which added to their anxiety. However, some workers found that the new arrangement actually enhanced their productivity. Without the burden of a long commute, they had more time and energy. Companies began to acknowledge that remote work could complement traditional office arrangements rather than replace them entirely.",
    wordIds: [157, 9, 36, 123, 85, 140, 19, 153, 44, 7, 70],
    level: "B2",
    topic: "work"
  },
  {
    id: 8,
    title: "The Election",
    text: "The coalition government faced a decisive election. Polls forecast a close race, and the outcome would determine the future of the country's democracy. The leading candidate made a bold declaration that she would eliminate corruption if elected. Her counterpart dismissed her promises as mere gestures designed to deceive voters. Media commentary on the campaign was extensive, and every statement was analyzed in detail. On election day, voter turnout exceeded expectations. The new leader was appointed with a clear mandate to implement reform, and the whole nation watched to see what would follow.",
    wordIds: [55, 107, 176, 112, 40, 108, 144, 97, 100, 106, 60, 168, 164, 23, 200],
    level: "B2",
    topic: "social"
  },
  {
    id: 9,
    title: "Protecting the Oceans",
    text: "Ocean conservation is one of the most fundamental challenges of our time. Plastic waste circulates through the water, causing fatal harm to marine life. Scientists have detected alarming levels of pollution in even the most remote areas. The erosion of coastal environments threatens communities that depend on fishing. Experts emphasize that the situation is crucial and requires a comprehensive global response. Several countries have already begun to enforce strict rules on waste disposal. If we do not confront this issue now, the damage may become impossible to reverse.",
    wordIds: [84, 185, 50, 172, 117, 158, 148, 101, 73, 151, 129, 80],
    level: "B2",
    topic: "nature"
  },
  {
    id: 10,
    title: "The Tech Startup",
    text: "A small group of engineers conceived an idea that would accelerate the way people learn languages. Their framework was simple: use cognitive science to create a curriculum that adapts to each student. They compiled research data and derived their approach from established learning theories. The exposure they gained at a technology conference was a major breakthrough. Investors were compelled by the authentic demonstration and agreed to fund the project. Within months, the app had emerged as the dominant platform in its category, and the team's frustration from years of hard work finally gave way to celebration.",
    wordIds: [75, 5, 182, 56, 102, 69, 115, 167, 43, 66, 34, 114, 146, 136, 183],
    level: "B2",
    topic: "technology"
  },
  {
    id: 11,
    title: "University Life Abroad",
    text: "When Maria first arrived at the university, she felt like her situation was chaotic. The bureaucracy involved in registering for classes was overwhelming, and the deadline for course selection was approaching fast. She had to grasp the new academic system quickly. Fortunately, a colleague from her program offered guidance and helped her evaluate which courses were eligible for credit transfer. Maria's determination to succeed never diminished. By the end of her first semester, she had not only adapted to university life but had also gained a deep appreciation for the experience of studying in a foreign country.",
    wordIds: [48, 45, 103, 188, 58, 189, 161, 143, 118, 120, 9, 24],
    level: "B2",
    topic: "education"
  },
  {
    id: 12,
    title: "A New Neighborhood",
    text: "The construction boom had transformed the old district beyond recognition. Where there had once been quiet streets, dramatic new buildings now stood tall. Some residents felt displaced by the changes and expressed contempt for the developers. Others applauded the modern apartments and new commerce that came with them. The local council tried to convey that development and heritage could exist side by side. They committed to preserving the authentic fabric of the neighborhood while allowing growth. The contradiction between old and new was not easy to resolve, but the community was determined to find a way forward.",
    wordIds: [41, 139, 128, 89, 22, 61, 94, 191, 34, 170, 90],
    level: "B2",
    topic: "daily_life"
  }

]
